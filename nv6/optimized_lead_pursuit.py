...
# --- Tham số Mô phỏng ---
VB_MAX_SPEED = 100.0          # Vận tốc tối đa của người đuổi (B)
SMOOTH_K = 8                  # Số lượng điểm dữ liệu dùng để hồi quy vận tốc
...
CLOSE_DIST_THRESHOLD = 40.0   # Khoảng cách kích hoạt pha tiếp cận cuối (Terminal Phase)
...
def estimate_velocity(pos: np.ndarray, time_arr: np.ndarray, i: int, k: int) -> np.ndarray:
    """Ước lượng vận tốc dựa trên k điểm dữ liệu gần nhất bằng hồi quy tuyến tính."""
    j = max(0, i - k + 1)
    tt = time_arr[j:i + 1]
    px = pos[j:i + 1, 0]
    py = pos[j:i + 1, 1]
...
def predict_intercept_point(pos_A: np.ndarray, pos_B: np.ndarray, vel_A: np.ndarray, vB_speed: float, max_iter: int) -> np.ndarray:
    """Dự đoán điểm đánh chặn (intercept point) bằng phương pháp lặp."""
    if vB_speed <= 1e-12:
        return pos_A.copy()
...
def compute_pursuer_velocity(pos_A: np.ndarray, pos_B: np.ndarray, pos_P: np.ndarray, vel_A: np.ndarray, dt: float) -> np.ndarray:
    """Tính toán vector vận tốc cho hệ thống bám sát."""
    ...
    # 1. CHIẾN LƯỢC BẮT DÍNH (Chống vọt lố/Zig-Zag)
    ...
    # 2. CHIA GIAI ĐOẠN ĐIỀU HƯỚNG
    if dist_ab <= CLOSE_DIST_THRESHOLD:
        # PHA CUỐI (TERMINAL PHASE): OVER-LEAD PURSUIT
        ...
        if tau > 0:
            OVERLEAD_FACTOR = 1.15 
            predicted_pos_A = pos_A + vel_A * (tau * OVERLEAD_FACTOR)
        ...
        # PHA XA (MID-COURSE PHASE)
        to_target = pos_P - pos_B
        dist_target = np.linalg.norm(to_target)
        if dist_target > 1e-12:
            desired_v = (to_target / dist_target) * VB_MAX_SPEED
        else:
            desired_v = np.zeros(2)
...
def simulate_one(file_path: str) -> dict:
    """Mô phỏng 1 lượt rượt đuổi và trả về kết quả thống kê."""
    ...
    n_steps = len(t)
    if n_steps == 0:
        raise ValueError(f"File rỗng hoặc không hợp lệ: {file_path}")
    ...
    for i in range(n_steps - 1):
        ...
        dt = t[i + 1] - t[i]
        desired_v = compute_pursuer_velocity(A_raw[i], B[i], P[i], V[i], dt)
...
def analyze_results(sim_results: list) -> None:
    """Đánh giá và in thống kê tổng hợp."""
    success_times = []
    failed_files = []
    fail_counts = {"0-249": 0, "250-499": 0, "500-749": 0, "750-999": 0}
    ...
    for range_name, count in fail_counts.items():
        print(f"Khoảng {range_name:<7}: {count} files")
...
if __name__ == "__main__":
    if not FILES:
        ...
        # In ra một số kết quả đầu tiên để xem lướt
        print("Trích xuất kết quả các file đầu tiên:")
